...
| Path / files | Purpose |
|--------------|---------|
| `GET /dashboard` | `merchant_app.html` (shell + lazy JSON) — KPIs، **بطاقة إعداد التاجر** (`merchant_setup_experience_v1`)، سلال، VIP، واتساب. `GET /dashboard/analytics` → `dashboard_v1.html` مع نفس البطاقة. |
