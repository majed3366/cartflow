...
| `static/cartflow_widget.js` | **Legacy monolith** (`B`): storefront path when **`CARTFLOW_WIDGET_RUNTIME_V2`** is omitted/false in **`widget_loader`**; **`GET /dev/widget-test*`** serves **`main._DEV_LEGACY_WIDGET_HARNESS_HTML`** (**`ENV=development`** only — **not** in **`_DEV_ROUTES_ALLOWED_WHEN_NOT_DEVELOPMENT`**). Keep for rollback, dev harness, and static/operational probes. **`/demo/store*`** stays **layered V2**. |
