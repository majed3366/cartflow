...
After **process restart**, the in-process task is gone; **startup** runs `run_recovery_resume_scan_async` → `_claim_schedule_row` → `_execute_resume_recovery_task` → `_run_recovery_sequence_after_cart_abandoned(..., delay_seconds=0.0, resume_from_durable_schedule=True)`.
