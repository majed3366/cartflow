...
| **Widget maintainability (static)** | `widget_loader.js` / `cartflow_widget.js` are scanned for **single-flight guards** (`__CARTFLOW_*__`) and **exit intent** markers — no browser execution. (**Note:** V2 storefronts chain `cartflow_widget_runtime/` when `CARTFLOW_WIDGET_RUNTIME_V2` is active; layered code **does not** inject `cartflow_widget.js`. Tests here still probe the shim + legacy monolith for rollback and observability parity.)
