# -*- coding: utf-8 -*-
"""
Founder evidence — Founder UX Closure V1.1.

Local lab R17, 390px, exactly 5 shots. DEPLOY: NO.
"""
...
DESKTOP = Path(
    r"C:\Users\Toshiba\Desktop\CartFlow_Founder_Review\Founder_UX_Closure_V1_1"
)

SHOTS = [
    "01_home_language_consistent_mobile.png",
    "02_workspace_language_consistent_mobile.png",
    "03_execution_cta_navigation_affordance.png",
    "04_settings_shipping_delivery_visible.png",
    "05_accepted_state_language.png",
]
...
def main() -> int:
    ...
    db_path = os.path.join(
        tempfile.gettempdir(),
        "cartflow_founder_ux_closure_v1_1_%s.db" % os.getpid(),
    )
    ...
    env["SECRET_KEY"] = "founder-ux-closure-v1-1-local"
    ...
    try:
        ...
        with sync_playwright() as p:
            ...
            try:
                page.wait_for_selector(".cf2-home__title, .cf-cda", timeout=15000)
            except Exception:
                pass
            snap(SHOTS[0])
            ...
            ws_cda = page.locator("#cf2-workspace-root .cf-cda").first
            ws_cda.wait_for(state="visible", timeout=25000)
            ws_cda.evaluate(
            ...
            exec_btn = page.locator(
                "#cf2-workspace-root [data-cf2-mission-exec='settings']"
            ).first
            exec_btn.wait_for(state="visible", timeout=15000)
            exec_btn.evaluate(
            ...
            try:
                page.wait_for_selector("#ma-rec-mission-pair, #cf2-rec-reasons", timeout=15000)
                pair = page.locator("#ma-rec-mission-pair")
                if pair.count() and pair.first.is_visible():
                    pair.first.scroll_into_view_if_needed()
                else:
  ...
  if (root && window.CartFlowUiV2Workspace) {
    await window.CartFlowUiV2Workspace.loadAndPaint(root);
            ...
            page.wait_for_timeout(1800)
            try:
                page.locator(".cf2-mission__status, .cf-cda").first.scroll_into_view_if_needed()
            except Exception:
                pass
            page.wait_for_timeout(400)
            snap(SHOTS[4])
