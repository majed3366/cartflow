...
DESKTOP = Path(r"C:\Users\Toshiba\Desktop\CartFlow_Founder_Review\Products_V1")

SHOTS = [
    "01_products_first_view_mobile.png",
    "02_product_attention_mobile.png",
    "03_product_numeric_truth_mobile.png",
    "04_product_purchase_vs_cart_mobile.png",
    "05_product_exposure_truth_mobile.png",
...
def main() -> int:
    ...
    db_path = os.path.join(
        tempfile.gettempdir(),
        "cartflow_products_v1_%s.db" % os.getpid(),
    )
    ...
    env["SECRET_KEY"] = "products-v1-founder-review"
    ...
    try:
        ...
        with sync_playwright() as p:
            ...
            page.wait_for_selector(
                '#cf2-products-root [data-cf2-prd="1"]', timeout=25000
            )
            page.wait_for_timeout(800)
            snap(SHOTS[0])

            page.locator('[data-cf2-prd-primary="1"]').first.evaluate(
                "el => el.scrollIntoView({block: 'start', inline: 'nearest'})"
            )
            page.wait_for_timeout(400)
            snap(SHOTS[1])

            amber = page.locator('[data-cf2-prd-id="nf-amber-night"]').first
            if amber.count():
                amber.evaluate(
                    "el => el.scrollIntoView({block: 'center', inline: 'nearest'})"
                )
            else:
                page.locator("[data-cf2-prd-card='1']").nth(1).evaluate(
                    "el => el.scrollIntoView({block: 'center', inline: 'nearest'})"
                )
            page.wait_for_timeout(400)
            snap(SHOTS[2])
            ...
            lab_card = page.locator(
                '[data-cf2-prd-id="nf-amber-night"] [data-cf2-prd-exposure="LAB_SYNTHETIC"]'
            ).first
            if not lab_card.count():
                lab_card = page.locator(
                    '[data-cf2-prd-exposure="LAB_SYNTHETIC"]'
                ).nth(1)
            lab_card.wait_for(state="visible", timeout=10000)
            lab_card.evaluate(
                "el => el.closest('[data-cf2-prd-card]')"
            ...
            missing = page.locator(
                '[data-cf2-prd-id="nf-missing-name"]'
            ).first
            if missing.count():
                missing.evaluate(
                    "el => el.scrollIntoView({block: 'center', inline: 'nearest'})"
