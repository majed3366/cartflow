# -*- coding: utf-8 -*-
"""
Founder evidence — Founder UX Closure V1.2.

Local lab R17, 390px, exactly 3 shots. DEPLOY: NO.
"""
...
DESKTOP = Path(
    r"C:\Users\Toshiba\Desktop\CartFlow_Founder_Review\Founder_UX_Closure_V1_2"
)

SHOTS = [
    "01_settings_shipping_delivery_single.png",
    "02_execution_confirm_language.png",
    "03_execution_cta_final.png",
]
...
def main() -> int:
    ...
    db_path = os.path.join(
        tempfile.gettempdir(),
        "cartflow_founder_ux_closure_v1_2_%s.db" % os.getpid(),
    )
    ...
    env["SECRET_KEY"] = "founder-ux-closure-v1-2-local"
    ...
    try:
        ...
        with sync_playwright() as p:
            ...
            page.wait_for_selector("#cf2-rec-reasons [data-cf2-rec-pick='shipping']", timeout=20000)
            page.locator("#cf2-rec-reasons").first.evaluate(
            ...
            exec_btn.wait_for(state="visible", timeout=25000)
            exec_btn.evaluate(
  ...
  if (root && window.CartFlowUiV2Workspace) {
    await window.CartFlowUiV2Workspace.loadAndPaint(root);
            ...
            page.locator(
                "#cf2-workspace-root [data-cf2-mission-act='confirm']"
