...
@router.post("/admin/ops/load-test/multi-store-cart-event")
def admin_load_test_multi_store_cart_event(
    request: Request,
    body: dict[str, Any] = Body(default_factory=dict),
) -> Any:
    """
    Safe multi-store cart-event load test (admin session required).
    Provisions loadtest-store-* virtual stores only; dry_run_whatsapp defaults true.
    """
    denied = _admin_json_auth_or_error(request)
    if denied is not None:
        return denied
