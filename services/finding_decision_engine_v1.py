    ...
    *,
    ...
    if ftype == TYPE_DOMINANT_HESITATION:
        ...
        return _decision(
            finding_id=fid,
            finding_type=ftype,
            decision=f"ركّز توضيحك هذا الأسبوع على معالجة سبب التردد الأعلى: {top or 'السبب المهيمن'}.",
            why=(
                f"الاستنتاج يُظهر سبباً مهيمناً بدعم الأدلة ({evidence}). "
                "التوضيح دون هذا التركيز يبعثر الجهد."
            ),
            impact="رفع احتمال إكمال الشراء عند العملاء المترددين لنفس السبب.",
            action=(
                "عدّل وضوح رسالة الشحن/التوصيل/قيمة المنتج (حسب السبب) في الودجت "
                "وقوالب الاسترجاع خلال نافذة المراجعة — دون تغيير الأسعار أو العروض."
            ),
            metric=(
