...
def record_purchase_lifecycle_closure(
    ...
    *,
    ...
    """
    Close lifecycle terminally for purchase — idempotent.
    """
    rk = (recovery_key or "").strip()
    if not rk:
        return
...
def record_purchase_lifecycle_closure_from_reply_intent(
    ...
    *,
    ...
    """Call after reply intent handling when intent is PURCHASE (read-only check)."""
    if (result.get("intent") or "").strip().upper() != INTENT_PURCHASE:
        return
