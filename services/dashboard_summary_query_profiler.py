# -*- coding: utf-8 -*-
"""
Per-function query + wall deltas for GET /api/dashboard/summary only.

Uses services.db_request_audit global request bucket (cursor counter) when enabled.
Produces [DASHBOARD SUMMARY SUBPROFILE] and [DASHBOARD SUMMARY TOP] — logging only.
...
_active: contextvars.ContextVar[bool] = contextvars.ContextVar(
    "ds_summary_query_prof_active", default=False
)
_stack: contextvars.ContextVar[Optional[List[Dict[str, Any]]]] = contextvars.ContextVar(
    "ds_summary_query_prof_stack", default=None
)
...
def dashboard_summary_query_profiling_active() -> bool:
...
def dashboard_summary_profile_span(fn: str) -> Iterator[None]:
    ...
    try:
        ...
        if popped is not frame:
            try:
                log.warning(
                    "[DASHBOARD SUMMARY PROFILER] stack mismatch fn=%s", (fn or "")[:120]
                )
            except OSError:
                pass
...
def _emit_dashboard_summary_profile_reports() -> None:
    ...
    for fn, st in rows:
        ...
        line = (
            f"[DASHBOARD SUMMARY SUBPROFILE] fn={fn} "
    ...
    for rank, (fn, st) in enumerate(rows[:top_n], start=1):
        ...
        line = (
            f"[DASHBOARD SUMMARY TOP] rank={rank} fn={fn} "
            f"queries={int(st.inclusive_queries)} "
            f"queries_exclusive={int(st.exclusive_queries)} "
            f"calls={int(st.calls)} "
            f"avg_queries_per_call={avg_q} "
