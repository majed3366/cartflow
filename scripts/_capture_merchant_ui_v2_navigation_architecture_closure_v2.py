# -*- coding: utf-8 -*-
"""Living Store — Navigation Architecture Closure V2 evidence (no App Bar section pill)."""
...
OUT = ROOT / "docs" / "product" / "merchant_ui_v2_navigation_architecture_closure_v2"
BASE = "https://smartreplyai.net"
MARKER = "nav-closure-v2"
...
PROBE = """() => {
  ...
  const chrome = document.querySelector('#cf2-section-chrome');
  ...
  return {
    ...
    chromeVisible: !!(chrome && !chrome.hidden && chrome.getBoundingClientRect().height > 2),
...
def main() -> None:
    ...
    with sync_playwright() as p:
        ...
        mpage.screenshot(path=str(OUT / "04_mobile_home_context_sheet.png"), full_page=False)
        mpage.click("#cf2-ctx-sheet-close")
        mpage.wait_for_timeout(400)

        mpage.click(".cf2-menu-btn")
        mpage.wait_for_timeout(600)
        evidence["probes"]["mobile_home_global_drawer"] = mpage.evaluate(PROBE)
        mpage.screenshot(path=str(OUT / "05_mobile_global_drawer.png"), full_page=False)
        ...
        mpage.screenshot(path=str(OUT / "06_mobile_workspace_closed.png"), full_page=False)

        if mpage.locator("#cf2-ctx-trigger").is_visible():
            mpage.click("#cf2-ctx-trigger")
            mpage.wait_for_timeout(600)
        evidence["probes"]["mobile_workspace_context"] = mpage.evaluate(PROBE)
        mpage.screenshot(path=str(OUT / "07_mobile_workspace_context_sheet.png"), full_page=False)
        mpage.click("#cf2-ctx-sheet-close")
        mpage.wait_for_timeout(400)
        evidence["probes"]["mobile_after_close"] = mpage.evaluate(PROBE)
        mpage.screenshot(path=str(OUT / "08_mobile_after_close_clean.png"), full_page=False)
