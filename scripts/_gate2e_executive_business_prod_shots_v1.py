# -*- coding: utf-8 -*-
"""Capture Gate 2E Desktop/Mobile Home + Workspace evidence."""
...
OUT = ROOT / "docs" / "product" / "gate_2e_executive_business_v1"


def main() -> int:
    OUT.mkdir(parents=True, exist_ok=True)
    uid = uuid.uuid4().hex[:10]
    email = f"cf.g2e.{uid}@smartreplyai.net"
    password = f"G2eLive!{uid[:8]}"
    ...
    with sync_playwright() as p:
        ...
        page.locator('input[name="store_name"]').fill(f"G2E {uid[:6]}")
        ...
        home_probe = page.evaluate(
            """async () => {
              ...
              return {
                ...
                executive_lang: !!(hes.governance||{}).executive_business_language,
        ...
        ws_probe = page.evaluate(
            """async () => {
              ...
              return {
                ...
                has_exec_subtitle: text.includes('أثر العمل') || text.includes('مركز قرار'),
                top_decision: top.decision_ar || top.title_ar || '',
                top_has_meaning: !!(top.business_meaning_ar),
                counter_as_title: /\\d+\\s*سلة/.test(top.decision_ar||'') && (top.decision_ar||'').includes('بلا رقم'),
    ...
    ok = (
        ...
        and bool(ws_probe.get("gate_2e"))
        and not ws_probe.get("counter_as_title")
    )
    payload = {
        **report,
        "generated_at_utc": datetime.now(timezone.utc).isoformat(),
        "sprint": "gate_2e_executive_business",
        "screenshots": {
            "desktop_home": "docs/product/gate_2e_executive_business_v1/after_desktop_home.png",
            "desktop_workspace": "docs/product/gate_2e_executive_business_v1/after_desktop_workspace.png",
            "mobile_home": "docs/product/gate_2e_executive_business_v1/after_mobile_home.png",
            "mobile_workspace": "docs/product/gate_2e_executive_business_v1/after_mobile_workspace.png",
        },
        "ok": ok,
        "status": (
