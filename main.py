...
from routes.commercial_decision_commitment_v1 import (  # noqa: E402
    router as commercial_decision_commitment_v1_router,
)
...
from routes.products_commercial_truth_v1 import (  # noqa: E402
    router as products_commercial_truth_v1_router,
)
...
app.include_router(products_commercial_truth_v1_router)
...
def _merge_recovery_settings_post_body(body: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if row is not None:
        ...
        if "vip_enabled" not in body:
            raw_ve = getattr(row, "vip_enabled", None)
            out["vip_enabled"] = True if raw_ve is None else bool(raw_ve)
        if "vip_notify_enabled" not in body:
            raw_vn = getattr(row, "vip_notify_enabled", None)
...
def api_dashboard_normal_carts(request: Request):
    ...
    if dashboard_snapshot_mode_enabled():
        payload = serve_enforced_snapshot_response(
            ...
            build_from_snapshot=build_normal_carts_from_snapshot,
...
def api_dashboard_widget_panel():
    ...
    try:
        ...
        from services.dashboard_snapshot_read_v1 import (
            _degraded_widget_panel_payload,
        ...
        if dashboard_snapshot_mode_enabled():
            payload = serve_enforced_snapshot_response(
                ...
                build_from_snapshot=build_widget_panel_from_snapshot,
...
def api_dashboard_refresh_state(request: Request):
    ...
    if dashboard_snapshot_mode_enabled():
        payload = serve_enforced_snapshot_response(
            ...
            build_from_snapshot=build_refresh_state_from_snapshot,
            degraded_builder=_degraded_refresh_state_payload,
            store_slug=(store_slug_hint or resolve_merchant_store_slug_for_snapshot()).strip(),
        )
        refresh_state_log_stage(
            "response_ready",
            store_slug=store_slug_hint or None,
...
def api_merchant_store_connection_status(request: Request):
    ...
    try:
        ...
        from services.dashboard_snapshot_read_v1 import (
            _degraded_store_connection_payload,
        ...
        if dashboard_snapshot_mode_enabled():
            payload = serve_enforced_snapshot_response(
                ...
                build_from_snapshot=build_store_connection_from_snapshot,
