...
class AdminOperationalHealthTests(unittest.TestCase):
    ...
    def test_page_renders_with_admin_session(self) -> None:
        ...
        self.assertIn('id="operational-verdict"', r.text)
        self.assertIn("الحالة التشغيلية", r.text)
        ...
        self.assertIn(payload_oc.get("verdict_tier"), ("ok", "watch", "action"))
        ...
        self.assertIn("أثر المتاجر", r.text)
        self.assertGreaterEqual(r.text.count("تفاصيل للدعم"), 5)
        ...
        self.assertIn("verdict_ar", payload_oc)
