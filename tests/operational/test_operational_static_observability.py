...
class OperationalStaticObservabilityTests(unittest.TestCase):
    def test_log_markers_exist_for_runtime_triage(self) -> None:
        ...
        self.assertIn("[RETURN TO SITE RECORDED]", main_src)
        self.assertIn("[RETURN STORE]", main_src)
        self.assertIn("[RETURN LOOKUP]", main_src)
        self.assertIn("[RETURN MATCH RESULT]", main_src)
        self.assertIn("[RETURN SOURCE]", main_src)
        self.assertIn("[RECOVERY RETURN CHECK]", main_src)
        self.assertIn("[RETURN TO SITE MATCHED RECOVERY]", main_src)
        self.assertIn("[RECOVERY STOPPED RETURNED_TO_SITE]", main_src)
